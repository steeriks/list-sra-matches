...
- **Level multi-select dropdown** — matched against `data-l`. Level names are per-ruleset, not global (IPSC: Level I/II/III, SRA: Club/Area/Nationals, Steel: Tier-1/2/3), so `buildLevels()` **rebuilds the options from whichever disciplines are currently selected** and is called on every discipline change. Selected levels the new discipline set doesn't offer are dropped — otherwise the table would empty out with no visible control explaining why. The whole dropdown hides when nothing in view declares a level (RESUL and IDPA report `--`). Sorted by standing via `LEVEL_ORDER`, not alphabetically, so `Level III` doesn't sort between `Level I` and `Level II`. Remembered under `sra_levels`. Note that the server-side `level`/`levels` arguments on `events()` return zero results — this has to be a client-side filter
